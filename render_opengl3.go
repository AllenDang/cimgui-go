...
func NewOpenGL3(io ImGuiIO, contentScale float32) (*OpenGL3, error) {
	err := gl.Init()
	if err != nil {
...
func (renderer *OpenGL3) Render(displaySize [2]float32, framebufferSize [2]float32, drawData ImDrawData) {
...
func (renderer *OpenGL3) SetFontTexture(image *RGBA32Image) {
	...
	io.GetFonts().SetTextureID(ImTextureID(uintptr(renderer.fontTexture)))
...
func (renderer *OpenGL3) LoadImage(image *image.RGBA) (ImTextureID, error) {
	texture, err := renderer.createImageTexture(image)
	if err != nil {
		return ImTextureID(uintptr(0)), err
	}
...
func (renderer *OpenGL3) createImageTexture(img *image.RGBA) (ImTextureID, error) {
	// Upload texture to graphics system
	var lastTexture int32
	var handle uint32
	gl.GetIntegerv(gl.TEXTURE_BINDING_2D, &lastTexture)
